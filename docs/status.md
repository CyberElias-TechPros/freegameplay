...
| **Vercel deployment** | `apps/web` builds clean and is configured for Vercel (`vercel.json`: framework preset, `/api/*` + `/media/*` rewrites, security + media cache headers). | Import the repo on Vercel, set `API_BASE` + `NEXT_PUBLIC_API_BASE` to the deployed worker URL and `NEXT_PUBLIC_SITE_URL` to the production domain. |
